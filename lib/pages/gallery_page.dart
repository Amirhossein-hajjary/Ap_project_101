...
class _GalleryPageState extends State<GalleryPage> {
  ...
  void _loadInitialPhotos() {
    setState(() {
      _displayedPhotos =
          _filteredAndSortedPhotos.take(_pageSize).toList();
    });
  }
  ...
  Future<void> _loadMorePhotos() async {
    ...
    if (mounted) {
      setState(() {
        final nextCount = _displayedPhotos.length + _pageSize;
        _displayedPhotos =
            _filteredAndSortedPhotos.take(nextCount).toList();
      ...
      if (file != null && mounted) {
        setState(() {
          _allPhotos.insert(0, {
            ...
            'name': 'Photo ${_allPhotos.length + 1}',
            'image': file.path,
            'album': album,
            'date': DateTime.now(),
            'isLocal': true,
        ...
        AuthService.showToast('Photo added to $album');
      }
    } catch (e) {
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ...
              if (name.isNotEmpty && !_userAlbums.contains(name)) {
                setState(() => _userAlbums.add(name));
                Navigator.pop(ctx);
                AuthService.showToast('Album "$name" created');
              } else if (_userAlbums.contains(name)) {
                AuthService.showToast('Album already exists',
                    isError: true);
              }
            },
  ...
  void _updatePhotoAlbum(int id, String newAlbum) {
    setState(() {
      final idx = _allPhotos.indexWhere((p) => p['id'] == id);
      if (idx != -1) _allPhotos[idx]['album'] = newAlbum;
      _loadInitialPhotos();
    });
    AuthService.showToast('Moved to $newAlbum');
  }
  ...
  void _bulkDelete() {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
            'Delete ${_selectedIndices.length} selected photo(s)?'),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(ctx),
              child: const Text('Cancel')),
            ...
            onPressed: () {
              ...
              setState(() {
                final ids = _selectedIndices
                    .map((i) => _displayedPhotos[i]['id'] as int)
                    .toList();
              ...
              AuthService.showToast('Photos deleted');
            },
          ...
          content: Wrap(
            ...
            children: [
              ChoiceChip(
                ...
                onSelected: (_) =>
                    setDialogState(() => selectedMoveAlbum = 'Default'),
              ),
              ..._userAlbums.map((a) => ChoiceChip(
                    label: Text(a),
                    selected: selectedMoveAlbum == a,
                    onSelected: (_) =>
                        setDialogState(() => selectedMoveAlbum = a),
                  )),
            ],
          ),
          actions: [
            ...
            TextButton(
              onPressed: () {
                setState(() {
                  ...
                  for (final id in ids) {
                    final idx =
                        _allPhotos.indexWhere((p) => p['id'] == id);
                    if (idx != -1) {
                      _allPhotos[idx]['album'] = selectedMoveAlbum;
                    }
                  }
                  _selectedIndices.clear();
                  _isSelectionMode = false;
  ...
  void _bulkShare() {
    final paths = _selectedIndices
        .map((i) => _displayedPhotos[i]['image'] as String)
        .toList();
  ...
  void _showAddOptions() {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setModalState) => GlassContainer(
          borderRadius: 30,
          child: Padding(
                  ...
                  children: [
                    ChoiceChip(
                      ...
                      onSelected: (_) => setModalState(
                          () => selectedTargetAlbum = 'Default'),
                    ),
                    ..._userAlbums.map((a) => ChoiceChip(
                          label: Text(a),
                          selected: selectedTargetAlbum == a,
                          onSelected: (_) =>
                              setModalState(() => selectedTargetAlbum = a),
                        )),
                  ],
                ),
      ...
      child: Column(
        ...
        children: [
          Icon(Icons.photo_library_outlined,
              size: 80, color: theme.hintColor.withAlpha(100)),
          ...
          Text('Tap + to add your first photo',
              style:
                  TextStyle(color: theme.hintColor.withAlpha(150))),
        ],
      ),
    );
  }
              ...
              onTap: () => Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (_) => ProfilePage(
                    ...
                    totalAlbums: _userAlbums.length,
                  ),
                ),
              ),
              child: Column(
